...
const communityData = [
  ...
  },
  {
    ...
    description: "Provide monthly support, education, and care for orphaned children in need.",
    raised: 28000,
    target: 50000,
    donors: 142,
    daysLeft: 12,
  ...
  },
  {
    ...
    description: "Emergency aid for families affected by recent earthquakes, providing shelter and basic necessities.",
    raised: 65000,
    target: 120000,
    donors: 298,
    daysLeft: 3,
    ...
    <View className="px-4 gap-4 mt-8">
      <View className="flex-row justify-between items-center">
        <View className="flex-row items-center">
          <Icon as={Heart} size={20} className="mr-2" stroke={selectedTheme.primary} />
                  ...
                  <View className="mb-4">
                    <View className="flex-row justify-between items-center mb-3">
                      <View className="flex-1">
                        <Text className="text-lg font-bold" style={{color: selectedTheme.secondary}}>
                          {formatCurrency(cause.raised)}
                        </Text>
                        <Text className="text-xs text-gray-500">raised so far</Text>
                      </View>
                    ...
                    <Progress 
                      ...
                      indicatorStyle={{backgroundColor: selectedTheme.primary}}
                    />
                  </View>

                  <View className="flex-row justify-between mb-3">
                    <View className="flex-1 rounded-lg p-2 mr-1" style={{backgroundColor: selectedTheme.primary + '10'}}>
                      <View className="flex-row items-center mb-1">
                        <View className="w-6 h-6 rounded-full items-center justify-center mr-1" style={{backgroundColor: selectedTheme.secondary + '20'}}>
                          <Icon as={Users} size={12} stroke={selectedTheme.primary} />
                        </View>
                        <Text className="text-xs text-gray-500 dark:text-foreground/80 flex-1" numberOfLines={1}>Donors</Text>
                      </View>
                      <Text className="text-sm font-bold" style={{color: selectedTheme.secondary}} numberOfLines={1}>
                        {cause.donors}
                      </Text>
                    </View>
                    
                    <View className="flex-1 rounded-lg p-2 mx-0.5" style={{backgroundColor: isDarkMode ? selectedTheme.primary + '10' : cause.daysLeft <= 7 ? '#FEF3C7' : '#F3F4F6'}}>
                      <View className="flex-row items-center mb-1">
                        <View className="w-6 h-6 rounded-full items-center justify-center mr-1" style={{backgroundColor: cause.daysLeft <= 7 ? '#F59E0B20' : '#6B728020'}}>
                          <Icon as={Clock} size={12} stroke={cause.daysLeft <= 7 ? '#F59E0B' : '#6B7280'} />
                        </View>
                        <Text className="text-xs text-gray-500 dark:text-foreground/80 flex-1" numberOfLines={1}>Days</Text>
                      </View>
                      <Text className="text-sm font-bold" style={{color: cause.daysLeft <= 7 ? '#F59E0B' : '#6B7280'}} numberOfLines={1}>
                        {cause.daysLeft}
                      </Text>
                    </View>
                    
                    <View className="flex-1 rounded-lg p-2 ml-1" style={{backgroundColor: isDarkMode ? selectedTheme.primary + '10' : '#ECFDF5'}}>
                      <View className="flex-row items-center mb-1">
                        <View className="w-6 h-6 rounded-full items-center justify-center mr-1" style={{backgroundColor: '#10B98120'}}>
                          <Icon as={Target} size={12} stroke={'#10B981'} />
                        </View>
                        <Text className="text-xs text-gray-500 dark:text-foreground/80 flex-1" numberOfLines={1}>Goal</Text>
                      </View>
                      <Text className="text-xs font-bold text-green-600" numberOfLines={1}>
